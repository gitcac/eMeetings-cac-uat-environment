$(function() {
    ...
    holder.ondragover = function() {
        $("#holder_helper").css("border","thick black dashed");
        return false;
    };
	holder.ondragleave = function() {
        $("#holder_helper").css("border","thick gray dashed");
        return false;
    };
    ...
    encodeButton.addEventListener('click', function(e) {
		...
		//validation
		fieldAtHand = $('#image-title').val();
		if (fieldAtHand == ""){
			$("#image-title").addClass("missingData");
			missingData = true;
			return false;
		}
		else{
			$("#image-title").removeClass("missingData");
		}
        ...
        var epoch = new Date();	//epoch time
		
		var x = null;
		
		try{
			x = parent.tinyMCE.activeEditor.getParam("meetingid");
		}catch(e){}
		
        jic.upload(result_image, 'upload.php', 'file', epoch.getTime()+'.'+output_format, callback, $('#image-title').val(), (x ? x : 1));
    }, false);
